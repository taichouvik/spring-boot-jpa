...
public class CustomBookRepo {
    ...
    public List<Book> getQueryResult(List<Filter> filters) {
        if (filters.size() > 0) {
            return bookRepo.findAll(getSpecificationFromFilters(filters));
        } else {
            return bookRepo.findAll();
        }
    }

    private Specification<Book> getSpecificationFromFilters(List<Filter> filter) {
        Specification<Book> specification = where(createSpecification(filter.remove(0)));
    ...
    private Specification<Book> createSpecification(Filter input) {
